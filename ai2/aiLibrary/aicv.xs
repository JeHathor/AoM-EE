...
extern float cvMilitaryEconSlider = 0.0;	 // Works in conjunction with Rush/Boom.  Settings near 1 will put a huge
											 ...
											 // Military/Econ at +1.0, Rush/Boom at +1.0:  Quick jump to age 2, rush mil/def with almost no vill production.
											 // Military/Econ at +1.0, Rush/Boom at -1.0:  Late to age 2, normal to age 3 with small military, grab 2 more settlements, then all military.
											 // Military/Econ at -1.0, Rush/Boom at +1.0:  Jump quickly to age 2, then jump quickly to age 3, delay upgrades and military.
											 // Military/Econ at -1.0, Rush/Boom at -1.0:  Almost no military until all settlements are claimed.  Extremely risky boom.
...
extern bool cvFixedStrategy = false;		// Set 'true' if you use custom sliders that should remain unchanged.

extern int cvMasterDifficulty = -1;	//0 = easy, 1 = medium , 2 = hard, 3 = nightmare (-1 means use world settings)
extern float cvMasterHandicap = 0.00;	//Only works in single player mode!  (Does not override at or below a factor of 0.00)

// Major god choice for the use with CivMods only. @ROTG
extern int cvCivSelectorTech = -1;
...
// AIAutoBalance: The AI will not try to overwhelm it's opponent. Instead it will try to match him. (Use for Scenarios!)
extern bool cvAiAutoBalance = false;
...
extern bool cvOkToFarmEarly = false;

extern int cvGoldBaseToUse = -1;
...
// ShowAiEcho:  Set this to false as long as you don't want to edit the script.
extern bool cvShowAiEcho = false;	//Suppress debug reports.
...
extern bool cvHuntMap = false;	// Set this true to make the AI optimize it's build for a map with lots of hunt.

extern bool cvNoRushMap = false;	//Set this true if the map should prevent rushing.

extern bool cvMigrationMap = false;	//Map already has a transport.
extern bool cvExpansionMap = false;	//Map provides no initial transport.
extern bool cvNomadMap = false;		//Map starts without a towncenter.
...
extern bool cvFishMap = false;  // Set this to true to make the AI think this is a fishable map.  Setting this false will *not* 
					...
					*/
extern bool cvIsPocket = false;
extern bool cvIsFlank = false;
...
//==============================================================================
int findUnit(int playerID=0, int state=cUnitStateAny, int unitTypeID=-1, bool visible=false)
{
   int count=-1;
